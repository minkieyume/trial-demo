...
(defmethod switch-face ((player <player>) (face T))
  (when (or (and face (typep face 'symbol))
	    (typep face 'string)
	    (typep face 'character))
    (setf (animation player) (symbolicate '-walk- face))))

(defmethod dir->face ((dir vec4))
  (let ((x (abs (vx dir)))
	(y (abs (vy dir))))
    (cond
      ((and (< x 0.01) (< y 0.01)) nil)
      ((> x y)
       (if (> (vx dir) 0) 'right 'left))
      (t
       (if (> (vy dir) 0) 'up 'down)))))
...
(defmethod handle :after ((ev tick) (player <player>))
  (let ((dt (dt ev))
    ...
    (switch-face player (dir->face movement))
    (cond ((and *movingp* dzerop) (setf *movingp* nil)
	   (idle player))
	  ((and (not *movingp*) (not dzerop)) (setf *movingp* t)
	   (walk player)))
